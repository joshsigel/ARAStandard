...
export async function generateMetadata({ params }: DomainPageProps): Promise<Metadata> {
  ...
  return {
    title: `Domain ${domain.id}: ${domain.title} — ARA Standard v1.1`,
...
function EvalMethodLabel({ method }: { method: string }) {
  ...
  return (
    <span className="inline-flex items-center gap-1.5 text-xs">
      ...
      <span className="text-muted">{labels[method] ?? method}</span>
    </span>
  );
}
    ...
    <div className="max-w-[1400px] mx-auto px-6 py-12">
      <Breadcrumb
        items={[
          ...
          { label: 'Domains', href: '/standard/v1.1/domains' },
      ...
      <div className="max-w-[72ch]">
        ...
        <div className="mt-12 pt-8 border-t border-border flex items-center justify-between">
          {prevDomain ? (
            <Link
              href={`/standard/v1.1/domains/${prevDomain.slug}`}
          ...
          {nextDomain ? (
            <Link
              href={`/standard/v1.1/domains/${nextDomain.slug}`}
